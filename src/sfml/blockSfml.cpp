...
BlockSfml::BlockSfml(Level * level, int spriteSize, int tileX, int tileY)
{
    ...
    else if (path == "0111" && !level->getBlock(tileX-1, tileY-1)->isSolid() && !level->getBlock(tileX-1, tileY+1)->isSolid()) path += "_alt";
    else if (path == "1011" && !level->getBlock(tileX+1, tileY-1)->isSolid() && !level->getBlock(tileX+1, tileY+1)->isSolid()) path += "_alt";
    else if (path == "1110" && tileY-1>=0 && !level->getBlock(tileX, tileY-1)->isSolid()) path += "_alt";
    else if (path == "1101" && tileY+1<level->getHeight() && !level->getBlock(tileX, tileY+1)->isSolid()) path += "_alt";
